...
def test_user_create_adds_row(session: Session) -> None:
    u = user_create(
        session, jaccount="alice", email="alice@sjtu.edu.cn", display_name="Alice"
    )
...
def test_course_create_adds_row(session: Session) -> None:
    c = course_create(
        session, code="VE477", title="Algorithms", semester="2026-fall"
    )
...
def test_enroll_links_user_and_course(session: Session) -> None:
    user_create(
        session, jaccount="bob", email="bob@sjtu.edu.cn", display_name="Bob"
    )
    course_create(
        session, code="ECE4721J", title="Systems", semester="2026-spring"
    )
    e = enroll(
        session, jaccount="bob", course_code="ECE4721J", role=Role.student
    )
...
def test_enroll_is_idempotent(session: Session) -> None:
    user_create(
        session, jaccount="carol", email="carol@sjtu.edu.cn", display_name="Carol"
    )
    course_create(
        session, code="ECE4721J", title="Systems", semester="2026-spring"
    )
...
def test_token_issue_stores_sha256_hash_and_returns_plaintext(
    ...
    user_create(
        session, jaccount="dave", email="dave@sjtu.edu.cn", display_name="Dave"
    )
...
def test_token_issue_revokes_prior_active_token(session: Session) -> None:
    user_create(
        session, jaccount="eve", email="eve@sjtu.edu.cn", display_name="Eve"
    )
...
def test_token_revoke_by_prefix(session: Session) -> None:
    user_create(
        session, jaccount="frank", email="frank@sjtu.edu.cn", display_name="Frank"
    )
