...
- **LLM client** (Bun single binary) — runs on the student's machine. OpenAI- and Anthropic-compatible local APIs; forwards to a student-configured upstream (the student's own LLM key or a ChatGPT/Codex subscription); uploads the response JSON to the student's jbox via a WebDAV PUT to the local Tbox endpoint.
